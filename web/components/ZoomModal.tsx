...
export default function ZoomModal({ src, alt, onClose, caption, onPrev, onNext, sidebar, zIndex = 400 }: ZoomModalProps) {
  // displayZoom is React state only for UI (zoom %, cursor style, draggable).
  // The actual transform is driven by animState + direct DOM writes — no re-render per frame.
  ...
  const applyTransform = useCallback((zoom: number, panX: number, panY: number) => {
    ...
    if (imgRef.current) {
      imgRef.current.style.transform = `translate(${panX}px, ${panY}px) scale(${zoom})`;
    }
  }, []);
  ...
  const reset = useCallback((animated = false) => {
    ...
    const controls = animate(0, 1, {
      ...
      ease: ZOOM_EASE,
      onUpdate: (t) => {
        const z = startZoom + (1 - startZoom) * t;
        applyTransform(z, startPanX * (1 - t), startPanY * (1 - t));
        setDisplayZoom(z);
      },
      onComplete: () => {
        applyTransform(1, 0, 0);
        ...
        controls: {
          const controls = animate(0, 1, {
            ...
            onUpdate: (t) => {
              ...
              const panX = cx - (cx - startPanX) * (z / startZoom);
              const panY = cy - (cy - startPanY) * (z / startZoom);
              applyTransform(z, panX, panY);
              setDisplayZoom(z);
            },
            onComplete: () => {
              const panX = cx - (cx - startPanX) * (targetZoom / startZoom);
              const panY = cy - (cy - startPanY) * (targetZoom / startZoom);
              applyTransform(targetZoom, panX, panY);
  ...
  return createPortal(
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`fixed inset-0 bg-black/90 backdrop-blur-sm overflow-hidden ${sidebar ? "sm:flex sm:flex-row" : "flex items-center justify-center"}`}
      style={{ zIndex, cursor: displayZoom <= 1 ? undefined : isDragging ? "grabbing" : undefined }}
    ...
    >
      <div className={sidebar ? "absolute inset-0 sm:relative sm:inset-auto sm:flex-1 sm:h-auto flex items-center justify-center overflow-hidden pt-10 px-10 sm:p-6 sm:pb-6" : "relative flex items-center justify-center w-full h-full"} style={sidebar && isMobileView ? { paddingBottom: `${sidebarHeight + 16}px` } : undefined}>
        <img
          ...
          onMouseDown={displayZoom > 1 ? handleMouseDown : undefined}
          onDoubleClick={(e) => { e.stopPropagation(); reset(true); }}
          onClick={(e) => e.stopPropagation()}
          style={{
            transformOrigin: "center center",
            cursor: displayZoom <= 1 ? "default" : isDragging ? "grabbing" : "grab",
            maxHeight: "100%",
            maxWidth: "100%",
            borderRadius: "8px",
            objectFit: "contain",
          ...
          <div className="absolute sm:bottom-6 left-1/2 -translate-x-1/2 glass rounded-xl px-4 py-2 flex items-center gap-3" style={{ bottom: isMobileView ? `${sidebarHeight + 24}px` : undefined }}>
            ...
            {displayZoom > 1 && (
              <>
                ...
                <span className="text-xs text-text-secondary/40">{Math.round(displayZoom * 100)}%</span>
                <button
